<?php
/**
 * @file
 * Language file for "paypal_donation" plugin.
 */

define("LAN_PAYPAL_DONATION_ADMIN_01", "Settings");
define("LAN_PAYPAL_DONATION_ADMIN_02", "Sandbox Mode");
define("LAN_PAYPAL_DONATION_ADMIN_03", "Disabled (live)");
define("LAN_PAYPAL_DONATION_ADMIN_04", "Enabled (sandbox)");
define("LAN_PAYPAL_DONATION_ADMIN_05", "Primary Email Address (live)");
define("LAN_PAYPAL_DONATION_ADMIN_06", "Primary Email Address (sandbox)");
define("LAN_PAYPAL_DONATION_ADMIN_07", "Validate Primary Email Address");
define("LAN_PAYPAL_DONATION_ADMIN_08", "Validate Primary Email Address on IPN messages");
define("LAN_PAYPAL_DONATION_ADMIN_09", "Logging failed IPNs");
define("LAN_PAYPAL_DONATION_ADMIN_10", "Logging failed IPNs using system logs.");

define('LAN_PAYPAL_DONATION_ADMIN_11', 'Donation items');
define('LAN_PAYPAL_DONATION_ADMIN_12', 'Create donation item');

define('LAN_PAYPAL_DONATION_ADMIN_13', 'ID');
define('LAN_PAYPAL_DONATION_ADMIN_14', 'Title');
define('LAN_PAYPAL_DONATION_ADMIN_15', 'Description');
define('LAN_PAYPAL_DONATION_ADMIN_16', 'Status');
define('LAN_PAYPAL_DONATION_ADMIN_17', 'Active');
define('LAN_PAYPAL_DONATION_ADMIN_18', 'Inactive');
define('LAN_PAYPAL_DONATION_ADMIN_19', 'Options');
define('LAN_PAYPAL_DONATION_ADMIN_20', 'Allow custom amount?');
define('LAN_PAYPAL_DONATION_ADMIN_21', 'Goal amount');
define('LAN_PAYPAL_DONATION_ADMIN_22', 'Goal date');
define('LAN_PAYPAL_DONATION_ADMIN_23', 'Currency');

define('LAN_PAYPAL_DONATION_ADMIN_24', 'Amount items');
define('LAN_PAYPAL_DONATION_ADMIN_25', 'Create amount item');

define('LAN_PAYPAL_DONATION_ADMIN_26', 'Donation item');
define('LAN_PAYPAL_DONATION_ADMIN_27', 'Amount label');
define('LAN_PAYPAL_DONATION_ADMIN_28', 'Amount value');

define('LAN_PAYPAL_DONATION_ADMIN_29', '');
define('LAN_PAYPAL_DONATION_ADMIN_30', '');
